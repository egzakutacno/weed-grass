#!/usr/bin/env python3
"""
Test script for meme-style description generation - V2.
Creates witty, sarcastic descriptions in the style of Ricky Gervais scrolling Reddit.
"""
...
def generate_meme_description(post_title, subreddit, score, image_url=None):
    """Generate a sarcastic, witty description using OpenAI in Ricky Gervais style."""
    try:
...
"""
        ...
        # Create a sarcastic, fed-up prompt
        prompt = f"""
Look at this Reddit post that's been flagged and figure out what's actually going on. Don't just summarize it—spot the absurdity, the irony, the stupidity. Then make a short, sharp, witty description like a human fed up with everything, smart but smug, kind of like Ricky Gervais would say if he was scrolling Reddit. Keep it punchy, sometimes let small grammar mistakes slip in where it feels natural, like a real person typing quickly. If it can't make sense of the post, just fall back to a simple line, but otherwise it should feel clever, annoyed, and a little exasperated with the world.

NO EMOJIS. Keep it under 3 sentences. Make it sound like someone who's seen it all and is mildly amused by how ridiculous everything is.

Post Context:
{context}

Generate a witty, sarcastic description:
"""
        
        response = openai.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a sarcastic, witty observer who sounds like Ricky Gervais scrolling Reddit. You're smart, smug, fed up with everything, and find the absurdity in situations. You write punchy, clever descriptions with occasional natural grammar mistakes. No emojis ever."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=120,
            temperature=0.9  # High creativity for natural, sarcastic tone
        )
        ...
        return f"Another day, another thing that makes you question everything. This is why we can't have nice things."
...
def test_meme_description():
    ...
    print("🎭 Testing Sarcastic Meme Description Generation")
    print("=" * 50)
    ...
    print("\n🎭 Generating sarcastic meme description...")
